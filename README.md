...
- 支持退格回退显示进度
- 支持粘贴批量显示（可配置倍数）
- 选区模拟写：在编辑器中框选一段代码后右键“模拟写代码（选区）”，选中内容立即清空，并在原位置逐字显现（选区模式每次按键固定显示 1 个字符）
- 多文件并发会话，标签页独立
- 完成时可提示信息
- 不修改源文件，仅在内存中操作
...
- 在编辑器里框选需要模拟写的代码，右键选择“模拟写代码（选区）”，选中的文本会被清空；随后每次按键只显现 1 个字符，直到选区内容完整呈现。停止会话不会关闭该文件。
...
- `simwrite.simulateSelection`：模拟写代码（选区）
...
- 选区模式说明：为保证演示节奏，选区模式每次按键固定显现 1 个字符，不受 `charactersPerKeystroke` 影响

## 进阶说明

- 虚拟文档在内存创建，并根据您的输入事件替换为源文件前缀内容
- 插入、删除与粘贴分别计算增减量以推动显示进度
- 选区模式仅替换选区起点到当前已显现位置的内容，保持光标跟随；停止会话不会关闭原文件标签页
...
[![Star History Chart](https://api.star-history.com/svg?repos=Arctic-Fox-W/Fake_coder_plugins&type=date&legend=top-left)](https://www.star-history.com/#Arctic-Fox-W/Fake_coder_plugins&type=date&legend=top-left)
